...
Before implementing ANY backend code, ALWAYS read these context files:
...
2. **CORE PATTERNS** - Always implement according to:
   - `context/core/essential-patterns.md` — Error handling, validation, security, logging patterns

3. **BACKEND-SPECIFIC** - Must follow:
   - `context/backend/domain-patterns.md` — Domain-driven design patterns
   - `context/backend/nestjs-clean-architecture.md` — NestJS architectural conventions

4. **TOOLING** - When relevant to implementation:
   - `context/tooling/biome-bun-docker.md` — Linting, formatting, build setup

**CRITICAL**: Use Read tool to load these files before writing any code. Your implementation MUST align with established patterns.
