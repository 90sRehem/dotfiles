...
Before writing or executing ANY tests, ALWAYS read these context files:

1. **MANDATORY** - Project Foundation:
   - `context/project/project-context.md` — Project patterns, testing conventions

2. **CORE TESTING PATTERNS** - Must follow:
   - `context/core/essential-patterns.md` — Error handling patterns, validation patterns to test

3. **TOOLING** - Testing framework setup:
   - `context/tooling/biome-bun-docker.md` — Test runners, build setup, CI patterns

4. **DOMAIN-SPECIFIC** - Read based on code under test:
   - Backend tests: `context/backend/domain-patterns.md`, `context/backend/nestjs-clean-architecture.md`
   - Frontend tests: `context/frontend/react-compound-patterns.md`, `context/frontend/react-query-patterns.md`

**CRITICAL**: Use Read tool to load relevant context before designing test strategy. Your tests MUST validate the patterns and conventions specified in context files.
